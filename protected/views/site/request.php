          ...
          <div class="col-lg-2 text-left">
            <button class="btn w-lg cancel-btn btn-lg {{(userCan('changeStatus_print') || userCan('changeStatus_lock') || userCan('reopen')) && userCan('delete') ? 'request-back' : ''}}" ng-click="cancel()" title="Abbrechen">Zur Übersicht</button>
					...
					<div class="col-lg-{{(userCan('changeStatus_print') || userCan('changeStatus_lock')) && userCan('reopen') ? '4' : '5'}} text-right">
						<button id="clear-position-fixed" ng-show="back" class="btn m-t-2 custom-btn btn-lg ion-skip-backward" ng-click="submitRequest();toTab(-1)" title="{{userCan('save') && back ? 'Speichern und ' : ''}}zurück"></button>
						...
						<button ng-show="next"  class="btn m-t-2 custom-btn btn-lg ion-skip-forward" ng-click="submitRequest();toTab(1)" title="{{userCan('save') && next ? 'Speichern und ' : ''}}weiter"></button>
         </div>  
				</div>
        </ng-form> 
			</div>
