...
<!--                        <button ng-show=" userCan('btnSenden', goal.status) && canEdit()" class="btn w-lg btn-lg custom-btn m-b-10" ng-click="submitForm( goal, 'submit')" title="Antragsteil zur Prüfung übermitteln">SENDEN</button>-->
                        <button ng-show=" userCan('btnAccept', goal.status) && canEdit()"  class="btn w-lg btn-lg btn-success m-b-10" ng-click="submitForm( goal, 'accept')" ng-class="{disabled: goal.newNotice.length && goal.status != 'rejected'}">AKZEPTIEREN</button>
