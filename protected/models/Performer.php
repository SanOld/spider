...
class Performer extends BaseModel {
  ...
  public $post = array();
  public $select_all = " tbl.*, CONCAT(CONCAT_WS(' ', NULLIF(tbl.plz, ''), NULLIF(tbl.city, '')), IF(tbl.address IS NOT NULL AND length(tbl.address) > 0, CONCAT(IF(length(tbl.city) > 0 OR tbl.plz > 0, ', ', ''), tbl.address), '')) full_address, CONCAT(checked_date, '%d.%m.%Y') checked_date_formatted, DATE_FORMAT(checked_date, '%d.%m.%Y') checked_date_formatted, CONCAT(usp.first_name, ', ', usp.last_name) representative_user";
