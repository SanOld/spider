var spi = angular.module('spi',['ngSanitize', 'ui.select', 'LocalStorageModule', 'ngTable', 'ui.bootstrap', 'ui.mask', 'ngAnimate']);
        ...
        network.get('hint', {page_code: code}, function (result, response) {
            if (result) {
                ...
                for(var i = 0; i < response.result.length; i++) {
                    if(!response.result[i].position_code) {
                        hints['_main'] = {title: response.result[i].title, text: response.result[i].description};
                    } else {
                        hints[response.result[i].position_code] = response.result[i].description;
                    }
                }
                callback(hints);
            }
